...
export async function viewObservability() {
  let spR, hR, aR, mR;
  try {
    [spR, hR, aR, mR] = await Promise.all([
  ...
  } catch (e) { return viewError("Failed to load observability", e.message); }
  ...
  const traces = {};
  for (const s of spans) (traces[s.trace_id] = traces[s.trace_id] || []).push(s);
  ...
  const alertRows = alerts.length === 0
    ...
    : alerts.map(a => rowItem({
        ...
        actions: a.resolved_at ? "" : `<button class="sm ghost" onclick="window.resolveAlert('${a.id}')">Resolve</button>`,
      })).join("");

  return `
    <div class="grid g4" style="margin-bottom:18px">
      <div class="card metric"><b>${spR.data ? spR.data.total : 0}</b><span>spans recorded</span></div>
      <div class="card metric"><b>${mR.data ? mR.data.total : 0}</b><span>events consumed</span></div>
    ...
    <div class="grid g2" style="margin-bottom:18px">
      <div class="card">
        ...
        <div class="hint">Components are healthy because their events keep arriving.</div>
        ...
        })).join("") || `<div class="empty">Components appear as events flow.</div>`}
      </div>
      <div class="card"><h3>Alerts</h3><div class="hint">Fired on failure events; resolve when handled.</div>${alertRows}</div>
    </div>
    <div class="grid g2">
      <div class="card"><h3>Recent traces</h3><div class="hint">Cross-service flows grouped by correlation id.</div>${traceRows}</div>
      <div class="card"><h3><span class="pulse-dot"></span>Activity stream <span class="tag">Module 11 consumer</span></h3><div class="hint">Every platform action on the Kafka bus.</div>
...
function viewError(title, msg) {
  return `<div class="error-box"><div class="err-title">${esc(title)}</div><div class="err-msg">${esc(msg)}</div><button onclick="window.go('observability')">Retry</button></div>`;
}