...
export async function viewWorkflows() {
  ...
  return `
    <div class="card" style="margin-bottom:18px">
      ...
      ${pipelines.length === 0 ? `<div class="empty">No pipelines defined yet.</div>` :
    ...
    <div class="grid g2">
      <div class="card"><h3>Executions</h3><div class="hint">Runs of the pipelines above.</div>
        ${execs.length === 0 ? `<div class="empty">No executions yet.</div>` :
          execs.slice(0, 12).map(x => rowItem({ title: "▶ " + esc((x.pipeline_id||"").slice(0,8)), meta: `started ${rel(x.created_at||x.started_at)}`, badges: badge(x.status||"running") })).join("")}</div>
      <div class="card"><h3>Human tasks</h3><div class="hint">Where executions wait for people. Decisions arrive from Supervision via Kafka.</div>
        ${tasks.length === 0 ? `<div class="empty">No human tasks.</div>` :
...
window.createPipeline = async function () {
  const name = $("wfName").value.trim(); if (!name) return;
  const r = await post(SVC.orchestration + "/pipeline", { name, steps: [{ id:"s1", name:"agent-work", type:"agent" }, { id:"s2", name:"human-signoff", type:"human_gate" }] });
...
function viewError(title, msg) {
  return `<div class="error-box"><div class="err-title">${esc(title)}</div><div class="err-msg">${esc(msg)}</div><button onclick="window.go('workflows')">Retry</button></div>`;
}