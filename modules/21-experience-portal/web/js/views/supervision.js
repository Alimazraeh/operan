// Supervision: manager inbox, approvals, escalations, interventions (Module 09).
  ...
  const inbox = items.length === 0
    ...
    : items.map(it => {
        const acts = it.item_type === "approval" && (it.status==="pending" || it.status==="in_progress")
          ? `<div style="display:flex;gap:6px"><button class="ok sm" onclick="window.supDecide('${it.item_id}','approve')">Approve</button><button class="bad sm" onclick="window.supDecide('${it.item_id}','reject')">Reject</button></div>` : "";
  ...
  return `
    ...
    <div class="grid g2">
      ...
      <div class="card">
        ...
        <div class="frow"><input id="escTitle" placeholder="e.g. prompt injection on support agent"><select id="escSev" style="max-width:110px"><option>low</option><option selected>medium</option><option>high</option><option>critical</option><option>p0</option></select><select id="escCat" style="max-width:130px"><option>security</option><option>compliance</option><option>operational</option><option>financial</option><option>hallucination</option><option>system</option></select><button class="sm warn" onclick="window.supEscalate()">Raise</button></div>
        <label style="margin-top:14px">Intervene on an agent</label>
        <div class="frow"><input id="ivAgent" placeholder="agent id or name"><select id="ivAction" style="max-width:120px"><option>pause</option><option>stop</option><option>restrict</option><option>suspend</option></select><button class="sm bad" onclick="window.supIntervene()">Apply</button></div>
...
window.supDecide = async function (id, action) {
  const body = action === "approve" ? { approver_id: uuid4(), comment: "Approved from the Operan portal" } : { rejector_id: uuid4(), reason: "Rejected from the Operan portal" };
...
window.supEscalate = async function () {
  const title = $("escTitle").value.trim(); if (!title) return;
  const r = await post(SVC.supervision + "/escalations", { severity: $("escSev").value, category: $("escCat").value, title, source_agent_id: uuid4() });
...
window.supIntervene = async function () {
  const target = $("ivAgent").value.trim(); if (!target) return;
  const r = await post(SVC.supervision + "/interventions", { action: $("ivAction").value, target_agent_id: target, reason: "Manager intervention from the Operan portal", duration_minutes: 60 });
