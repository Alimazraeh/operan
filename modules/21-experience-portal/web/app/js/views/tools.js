// Tools: registry + execution log (Module 08).
...
export async function viewTools() {
  let tR, eR;
  try {
    [tR, eR] = await Promise.all([get(SVC.tools + "/tools?page_size=50"), get(SVC.tools + "/executions?page_size=20")]);
  } catch (e) { return viewError("Failed to load tools", e.message); }
  ...
  return `
    <div class="card" style="margin-bottom:18px">
      ...
      ${tools.length === 0 ? `<div class="empty">No tools registered for this tenant.</div>` :
        tools.map(t => rowItem({
          title: "🛠 " + esc(t.name),
    ...
    <div class="card"><h3>Execution log</h3><div class="hint">Auditable record of every tool an agent has used.</div>
      ${execs.length === 0 ? `<div class="empty">No executions yet.</div>` :
        execs.map(x => rowItem({ title: esc(x.tool||x.tool_id||"tool"), meta: `by ${esc((x.agent_id||"").slice(0,12))} · ${rel(x.created_at||x.requested_at)}`, badges: badge(x.status||"completed") })).join("")}</div>`;
}

window.registerTool = async function () {
  const name = $("tlName").value.trim(); if (!name) return;
  const r = await post(SVC.tools + "/tools/register", { name, category: $("tlCat").value.trim()||"general", description: $("tlDesc").value.trim() });
...
window.execTool = async function (name) {
  const r = await post(SVC.tools + "/execute", { tool: name, agent_id: "portal-user", parameters: { invoked_from: "operan-portal" } });
  if (r.ok) toast("Executed "+esc(name), "ok"); else toast("Execution failed", "bad");
  window.go("tools");
};

function viewError(title, msg) {
  return `<div class="error-box"><div class="err-title">${esc(title)}</div><div class="err-msg">${esc(msg)}</div><button onclick="window.go('tools')">Retry</button></div>`;
}